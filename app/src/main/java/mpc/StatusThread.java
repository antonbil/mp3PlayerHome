	...
	private void checkPlayingStatus() throws IOException {
		...
		while((response = in.readLine()) != null){
			...
			if(response.startsWith("song: ")){
				...
				currentSong = Integer.parseInt(response.substring(6));
			}
			if(response.startsWith("time: ")){
				//Log.v("samba", response);
				currentTime = Integer.parseInt(response.substring(6).split(":")[0]);
			}
		}
		this.status = new MPCStatus(playing, shuffling, currentVol, currentSong, currentTime);
